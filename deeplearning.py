...
def extract_text(filename,image,bbox):
    x,y,w,h = bbox
...
def drawings(filename, image,boxes_np,confidences_np,index):
    # drawings
    text_list = 0
    for ind in index:
        ...
        license_text = extract_text(filename,image,boxes_np[ind])
        ...
        # text_list = normal_string
        
        for k in text_list.split("\n"):
...
# predictions
def yolo_predictions(filename, img,net):
    ## step-1: detections
    input_image, detections = get_detections(img,net)
    ## step-2: NMS
    boxes_np, confidences_np, index = non_maximum_supression(input_image, detections)
    ## step-3: Drawings
    result_img, text = drawings(filename, img,boxes_np,confidences_np,index)
...
def object_detection(path,filename):
    ...
    result_img, text_list = yolo_predictions(filename, image,net)
